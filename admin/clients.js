module.exports = function (app, connection1) {
                    ...
                    connection1.query('SELECT `client_service`.*, `services`.`service` as `ser_name`, `client`.`status` as `client_status`, `client`.`full_name` as `client_name`, `client`.`created_at` as `s_date`, `client`.`id` as `client_id`, `categorie`.`segment` as `cat_segment`, `categorie`.`name` as `cat_name` FROM `client_service` LEFT JOIN `services` ON `services`.`id` = `client_service`.`service_id` LEFT JOIN `client` ON `client`.`id` = `client_service`.`client_id` LEFT JOIN `categorie` ON `categorie`.`id` = `services`.`categorie_id` WHERE `client_service`.`client_id` =' + id, (err, dd) => {
                        ...
                        connection1.query('SELECT * FROM `service_and_group_id` WHERE service_group_id=' + req.body.group, (err, resultGroup) => {
                            ...
                            add_service.forEach(function (item, index) {
                                ...
                                connection1.query('INSERT INTO `client_service`  (`service_id`,`client_id`,`service_group_id`,`created_at`,`strategy`) VALUES ("' + item + '",' + id + ',"' + group_id + '","' + dformat + '","' + req.body.strategy[1] + '")', (err, result) => {
                                    console.log(err);
                                    //  console.log(result);
                                ...
                                connection1.query('SELECT * FROM `strategy_client` WHERE `client_id`=' + id + ' ORDER BY id DESC', (err, stgfind) => {
                                    ...
                                    // connection1.query('SELECT * FROM `client_service` WHERE `client_id`= ' + id + ' AND `strategy` ="' + item5 + '"', (err, result) => {
                                        ...
                                        result.forEach(function (item6, inedx) {
                                            connection1.query('UPDATE `client_service` SET  `strategy`= "' + stgfind[0].strategy + '" WHERE  `id` ="' + item6.id + '" AND `strategy`= "'+item5+'"', (err, result2) => {
    ...
    // app.post("/admin/client/mail", (req, res) => {
    //     const toEmail = req.body.to;
    //     console.log("to", toEmail)
    //     const subjectEmail = "User ID and Password";
    //     const htmlEmail = "<p>Dear </p><p>Thank you for choosing Smart Algo for Algo Platform. We are pleased to inform that the password of your <br> Algo Platform has been resetted as per details mentioned below:</p><p>Login Details:</p><p>User Name / User ID : <b></b><br>Login Password : <b></b></p><p>Note : Please Change Your Login Password as per your choice.</p>";
    //     const textEmail = ''
    //     var { CommonEmail } = require(`../Common/SendEmails/CommonEmail.js`)
    //         // console.log("dhfdf", require(`../Common/SendEmails/CommonEmail.js`).CommonEmail(toEmail, subjectEmail, htmlEmail, textEmail, res))

    //     // console.log("dhfdf", CommonEmail(toEmail, subjectEmail, htmlEmail, textEmail, res))
    //     CommonEmail(toEmail, subjectEmail, htmlEmail, textEmail, res)
    ...
    //         connection1.query('SELECT `broker_response`.`client_id` FROM `client` JOIN `broker_response` ON `broker_response`.`client_id` = `client`.`id` GROUP BY `broker_response`.`client_id`', (err, all_client) => {

    //             all_client.forEach(element => {
    //             console.log('ids -',element.client_id);


    //             connection1.query('SELECT `order_id`,`client_id` FROM `broker_response` WHERE `client_id` = "'+element.client_id+'" AND `order_id` IS NOT NULL AND `order_id` != "" AND `order_id`!= "undefined"', (err, client_order_id) => {



    //               if(client_order_id.length > 0 ){
    //                 console.log('valid client id -',client_order_id[0].client_id);
    //                 var dt = dateTime.create();
    //                 var ccdate = dt.format('Y-m-d H:M:S');
    //                 //console.log('valid client id -',client_order_id[0].client_id , 'time -',ccdate);

    //                    connection1.query('UPDATE `client` SET `not_change_broker` = "1" WHERE `id`="' + client_order_id[0].client_id + '"', (err, result1) => {
    //                      console.log('eroor query if-',err);

    //                 });


    //               }else{
    //                 console.log('eroor query else-');
